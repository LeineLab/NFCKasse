...
class Database:
	...
	def getProductAlias(self):
		if not self.connect():
			return []
		self.cursor.execute('SELECT a.ean, a.target, p.name FROM product_alias a LEFT JOIN products p ON a.target = p.ean ORDER BY p.name ASC')
		try:
			results = self.cursor.fetchall()
			return results
		except TypeError:
