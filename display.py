...
class Display:
    ...
    def showOptions(self, ok, cancel):
        ...
        self.draw.text(
            (0, self.HEIGHT - font_height), text, font=self.fontsmall, fill=(0, 200, 0)
        )
        text = cancel
        t1, t2, font_width, font_height = self.fontsmall.getbbox(text)
        self.draw.text(
            (self.WIDTH - font_width, self.HEIGHT - font_height),
            text,
            font=self.fontsmall,
            fill=(200, 0, 0),
        )
    ...
    def showQR(self, data):
        ...
        qr.add_data(data)
        img = qr.make_image(back_color=(0, 0, 0), fill_color=(255, 255, 255)).convert(
            "RGB"
        )
